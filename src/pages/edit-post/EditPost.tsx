...
function EditPost({ open,onClose, postToEdit }: Props) {
  ...
  return (
    <Dialog open={open} onClose={onClose} sx={{display: 'flex', flexDirection: 'column', alignItems: 'center',  "& .MuiPaper-root": {
      borderRadius: '1rem', padding: '20px'
    }, }}>
      <DialogTitle variant="h5" fontWeight="bold" color='secondary.main' align='center'>Edit Your Post</DialogTitle>
      <PostForm uploadPostDto={post} submitText='Edit Post' onSubmitFunc={editPost} />
    </Dialog>
  );
}
