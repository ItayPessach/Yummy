...
import {InputAdornment, TextField, Button, IconButton, Box, Stack, Typography} from "@mui/material";
import RestaurantMenuOutlinedIcon from "@mui/icons-material/RestaurantMenuOutlined";
import CloudUploadIcon from "@mui/icons-material/CloudUpload";
import ValidatedSelectCity from '@/components/forms/ValidatedSelectCity';
import { yupResolver } from "@hookform/resolvers/yup";
import * as yup from "yup";
import {ChangeEvent, useState, useRef} from "react";
import CancelIcon from "@mui/icons-material/Cancel";
import {config} from "@/config";
import {PostFormInput, UploadPostDto} from '@/common/types';
...
function PostForm({ sx, uploadPostDto, submitText, onSubmitFunc }: Props) {
  const [postImage, setPostImage] = useState<File | undefined>(uploadPostDto.picture);
  const [postImageError, setPostImageError] = useState('');
  const hiddenFileInput = useRef<HTMLInputElement>(null);

  const { control, handleSubmit, formState: { errors, isSubmitted } } = useForm<PostFormInput>({
    values: {
  ...
  return (
    <form style={sx} onSubmit={handleSubmit(onSuccessSubmit, onErrorSubmit)}>
      <Stack spacing={3} alignItems='center'>
        <input type='file' ref={hiddenFileInput} onChange={changeProfileImage} style={{display: 'none'}}/>
        <Stack spacing={1}>
          {postImage ? (
            ...
            <Box sx={{backgroundColor: 'white', borderRadius: '2rem', border: 'black dashed 2px', px: 2}}
              onClick={openFileInput}>
              <img width={200} height={200} src={config.publicFolderUrl + "add-photo.png"} alt="Preview"/>
            </Box>
          }
          {postImageError && <Typography variant="caption" sx={{
            fontWeight: "bold",
            mb: 5,
            color: "error.main"
          }}>{postImageError}</Typography>}
        </Stack>
        <Controller
          name="restaurant"
          control={control}
          render={({field}) =>
            ...
            />}
        />
        <ValidatedSelectCity control={control} sx={{height: "5vh", width: "30vw"}}/>
        <Controller
          name="description"
          control={control}
          render={({field}) =>
